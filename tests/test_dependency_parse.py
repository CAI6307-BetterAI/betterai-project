...
def test_alias_extraction():
    ...
    res_3 = tripleset.get_or_none(
        subject="htn", predicate="be", object="a medical condition", root=True
    )
...
def test_alias_assign_root_before_context():
    ...
    res_1 = tripleset.get_or_none(
        subject="hypertension", predicate="is", object="a medical condition"
    )
    assert res_1 is not None

    # Un-associate from original node
    res_2 = tripleset.get_or_none(subject="htn", predicate="is", object="a medical condition")
    assert res_2 is None

    # Allow access to node via root
    res_3 = tripleset.get_or_none(
        subject="htn", predicate="is", object="a medical condition", root=True
    )
    assert res_3 is not None
